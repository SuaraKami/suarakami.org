<script lang="ts">
  ...
  } from "@/components/force-graph/types";
  ...
  let _anchorEl = $state<HTMLElement | null>(null);
  let activeSlug = $state<string | null>(null);
  let detailSlug = $state<string | null>(null);
  const _graphData = $derived<ForceGraphData | null>(
    activeSlug ? buildGraphData(activeSlug) : null
  );
  const relations = $derived(activeSlug ? getRelations(activeSlug) : []);
  const _groupedRelations = $derived.by(() => {
    ...
    for (const relation of relations) {
      ...
      const item = {
        id: targetId,
        term: targetEntry
          ? targetEntry.data.term
          : (relation.type ?? "Tidak Diketahui"),
        entry: targetEntry,
      };
  ...
  const _rootEntry = $derived(
    activeSlug ? (entryMap.get(activeSlug) ?? null) : null
  );
  ...
  const _detailHeading = $derived(detailEntry?.data.term);
  const _detailDefinition = $derived(detailEntry?.data.definition);
  ...
  function _handleVisibilityChange(next: boolean) {
    if (!next) {
      resetActive();
    }
  }
  ...
  function _focusRelation(slug: string | null) {
    if (!slug) {
      return;
    }
    detailSlug = slug;
  ...
  function _handleGraphSelect(payload: GraphSelectionPayload) {
    if (!payload?.id) {
      return;
    }
...
>
  <Popover.Portal>
    <Popover.Content customAnchor={anchorEl} sideOffset={12} class="z-50">
      <div class="rounded-2xl border border-border/60 bg-panel">
...
{:else}
<Dialog.Root open={glossaryStates.isOpen} onOpenChange={handleVisibilityChange}>
  <Dialog.Portal>
...
<div class="space-y-4 text-sm">
  ...
  <div class={['grid gap-3', { 'md:grid-cols-2': !dense }]}>
    ...
    >
      ...
      <div
        class={`mt-2 ${graphHeight} min-h-56 rounded-lg border border-border/50 bg-panel/80 p-1`}
      >
        {#if graphData}
        <ForceGraph
          graphData={graphData}
  ...
  <div class="rounded-xl border border-border/60 bg-panel/70 p-3">
    ...
    <p class="mt-3 text-sm text-foreground-muted">
      Belum ada relasi tambahan untuk entri ini.
    </p>
    {/if}
  </div>
</div>
